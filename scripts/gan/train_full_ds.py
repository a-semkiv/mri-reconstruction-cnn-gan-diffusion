...
from scripts.common.logging.final_ds.gan import GANMetricsLoggerFinal
from scripts.common.generation_callbacks.final_ds.gan import (
    GANEpochGenerationCallbackFinal,
)
from scripts.common.plotting_callbacks.final_ds.gan import GANPlottingCallbackFinal
...
def main():
    ...
    metrics_cb = GANMetricsLoggerFinal(save_dir=logs_dir)

    plot_cb = GANPlottingCallbackFinal(
        metrics_path=logs_dir / "metrics.json",
        save_dir=log_root / "plots",
        model_name="Pix2Pix GAN (R=4) full dataset",
    )
    ...
    epoch_gen_cb = GANEpochGenerationCallbackFinal(
        batch=fixed_batch,
        save_dir=gen_dir,
        device=device,
    )
    ...
    model_cpu = model.generator.to("cpu")
    model_cpu.eval()

    weights_path = saved_model_dir / "generator_weights.pt"
    torch.save(model_cpu.state_dict(), weights_path)

    model_config = {
        "model_class": "Pix2PixGenerator (U-Net)",
        "in_chans": 1,
        "out_chans": 1,
        "chans": 32,
        "num_pool_layers": 4,
    ...
    print(f"Generator weights saved to {weights_path}")
